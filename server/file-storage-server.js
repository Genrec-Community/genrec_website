// File Storage Backend - Saves data to JSON files (no database needed)
...
console.log(`   📧 Contacts: ${contacts.length} entries`);
console.log(`   ⭐ Feedback: ${feedback.length} entries`);
console.log(`   🔄 Interactions: ${interactions.length} entries`);
console.log(`   💬 Conversations: ${conversations.length} entries`);
  ...
  try {
    ...
    if (path === '/api/admin/conversations' && method === 'GET') {
      sendJSON(res, 200, { 
        ...
        file_location: FILES.conversations
      });
      return;
    }
        ...
        data: {
          storage: {
            ...
            files: {
              ...
              interactions: FILES.interactions,
              conversations: FILES.conversations
            }
          },
          conversations: { 
    ...
    if (path === '/' && method === 'GET') {
      sendJSON(res, 200, {
        message: 'Genrec AI Backend - File Storage',
        status: 'Running',
        storage: {
          type: 'JSON Files',
          location: DATA_DIR,
          files: {
            contacts: `${contacts.length} entries in ${FILES.contacts}`,
            feedback: `${feedback.length} entries in ${FILES.feedback}`,
            interactions: `${interactions.length} entries in ${FILES.interactions}`,
            conversations: `${conversations.length} entries in ${FILES.conversations}`
          }
        },
        data_access: {
          view_files: 'Open the JSON files in the data/ folder',
...
// Start server
server.listen(PORT, () => {
  console.log(`🚀 Genrec AI Backend (File Storage) running on http://localhost:${PORT}`);
  ...
  console.log(`🔍 View data files: Open ${DATA_DIR} folder`);
...
process.on('SIGINT', () => {
  ...
  console.log(`📁 Data files saved in: ${DATA_DIR}`);
  process.exit(0);
});
